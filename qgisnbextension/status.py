...
def list_algorithms( provider_id ) -> None:
...
def show_alg_infos( algid ) -> None:
    ...
    if alg is None:
        print("Algorithm '%s' not found" % args.name, file=sys.stderr)
        return False
    ...
    for p in alg.parameterDefinitions():
        dflt = p.defaultValue()
        print('  {n: <25}{t: <20}{v: <10}{i: <10}{d}'.format(n=p.name(),d=p.description(),t=p.type(),
            i='dest' if p.isDestination() else '', v=dflt if dflt is not None else ''))
    return True


def processing_infos(argstring) -> None:
    """ Display processing informations
    """
    import argparse
    ...
    cmd.add_argument('-P','--providers', action='store_true', default=False, help="Show only providers", dest='providers')
    ...
    start_qgis_application(verbose=args.verbose, enable_processing=True)
